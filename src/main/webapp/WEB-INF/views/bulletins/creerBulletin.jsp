							...
							<div class="col-sm-12 col-md-7">
								<c:choose>
									<c:when test="${ not periodeOk }">
										...
										<span class="text-danger">Il n'y a pas cette
											p&#xE9;riode dans la base</span>
									</c:when>
									<c:otherwise>
								...
								<c:choose>
									<c:when test="${ not remEmplOk }">
										<form:select path="remunerationEmploye.id"
											name="remunerationEmploye" class="custom-select is-invalid"
											required="required">
											<form:options items="${ listeEmploye }" itemLabel="matricule"
												itemValue="id" />
										</form:select>
										<span class="text-danger">Il n'y a pas cette
											remuneration employ&#xE9; dans la base</span>
									</c:when>
									<c:otherwise>
										<form:select path="remunerationEmploye.id"
											name="remunerationEmploye" class="custom-select"
											required="required">
											<form:options items="${ listeEmploye }" itemLabel="matricule"
												itemValue="id" />
										</form:select>
								...
								<c:choose>
									<c:when test="${ not primeOk }">
										<form:input path="primeExceptionnelle" type="number"
											...
											id="colFormLabelLg" name="prime" min="0" step="0.01"
											value="0" required="required" />
										<span class="text-danger">la prime exceptionnelle doit
											être sup&#xE9;rieur ou &#xE9;gale à 0</span>
									</c:when>
									<c:otherwise>
										<form:input path="primeExceptionnelle" type="number"
											class="form-control form-control-lg" id="colFormLabelLg"
											name="prime" min="0" step="0.01" value="0"
											required="required" />
									</c:otherwise>
								</c:choose>
							</div>
